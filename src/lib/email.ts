...
interface MailAttachment { filename: string; b64: string; cid: string }
...
async function deliverEmail(
  ...
  if (smtpHost && smtpUser && smtpPass) {
    ...
    await withRetry(() => transporter.sendMail({
      ...
      to, subject, html,
      attachments: attachments.map(q => ({ filename: q.filename, content: Buffer.from(q.b64, 'base64'), cid: q.cid })),
    }), `smtp ${logTag}`)
...
export async function sendTicketEmail(params: TicketEmailParams): Promise<void> {
  ...
  // QR como anexo inline (CID). O Gmail NÃO renderiza data:image em e-mail.
  const qrParts = params.tickets.map((t, i) => ({
    filename: `ingresso-${i + 1}.png`,
    b64:      t.qrDataUrl.split(',')[1] ?? '',
    cid:      `qr-${i}`,
  }))
  await deliverEmail(params.to, `Seus ingressos — ${params.eventName}`, html, qrParts, `order=${params.orderId} to=${params.to}`)
}
...
function buildCancellationHtml(p: CancellationEmailParams): string {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F3EC;padding:32px 16px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        ...
        <tr>
          <td style="background:#1A211B;border-radius:0 0 16px 16px;padding:20px 32px;text-align:center;">
            <p style="margin:0 0 6px;font-size:12px;color:rgba(244,241,235,0.5);">Pedido <strong style="color:rgba(244,241,235,0.8);">#${p.orderId}</strong></p>
            <p style="margin:0;font-size:11px;color:rgba(244,241,235,0.35);">
              Moventis Tickets · <a href="https://moventistickets.com.br" style="color:rgba(31,107,78,0.8);text-decoration:none;">moventistickets.com.br</a>
            </p>
          </td>
